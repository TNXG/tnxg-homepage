        ...
        <div id="friend-card" class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6 p-4">
            <div v-for="friend in friends" :key="friend.id" class="flex">
                <a v-if="!friend.hide" :href="friend.url" target="_blank"
                    class="relative bg-white dark:bg-gray-800 rounded-lg shadow-lg overflow-hidden transform transition-transform duration-300 hover:scale-105 hover:shadow-2xl group flex flex-col w-full">
                    <div
                        class="flex items-center justify-center p-6 bg-gray-100 dark:bg-gray-700 relative transition-all duration-300">
                        <span
                    ...
                    <div class="px-6 py-4 bg-gray-100 dark:bg-gray-700 text-center">
